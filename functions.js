		...
		case 1: //medical
		{
			for (i in dbData) {
				if ((last_updated-dbData[i][4]) < last_update_cutoff_min * 60){
					...
					document.write('<div id="' + i + '" class="mark" style="' + getTopLeft(server_map, dbData[i][1], dbData[i][0], i) + dead + 'z-index:' + dbData[i][4] + ';" oncontextmenu="thisHide(this, box_hide_delay); return false;"><b>' + dbData[i][0] + '</b> [' + i + ']<br>' + dbData[i][3] + '<br><b>Humanity:</b> ' + Math.round(dbData[i][6]) + '<br><b>Blood:</b> ' + Math.round(medical[7] / 100) / 10 + ' pints' + ((medsum !='') ? '<br>' : '') + medsum + '</div>');
				}
			}
		}
		break;
		...
		case 3: //position
		{
			for (i in dbData){
				if ((last_updated - dbData[i][4]) < last_update_cutoff_min * 60){
					players_displayed ++;
					dead = (dbData[i][2] != '0') ? 'text-decoration:line-through;' : '';
					document.write('<div id="'+i+'" class="mark" style="'+getTopLeft(server_map,dbData[i][1],dbData[i][0],i)+dead+'z-index:'+dbData[i][4]+';" oncontextmenu="thisHide(this,box_hide_delay);return false;"><b>'+dbData[i][0]+'</b> ['+i+']<br>'+dbData[i][3]+'<br><input type="text" style="width:155px;font-size:10px;" onclick="this.focus();this.select();" value="'+dbData[i][11]+'"></div>');
				}
			}
		}
		break;
